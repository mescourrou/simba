...
pub fn closest_uint_modulo(numerator: f32, denumerator: f32) -> Result<u32, SimbaError> {
    ...
    if float_mod < 0. {
        return Err(SimbaError::new(SimbaErrorTypes::MathError, "closest_uint_modulo with negative division!"));
    }
...
pub fn round_precision(number: f32, precision: f32) -> Result<f32, SimbaError> {
    if precision <= 0. {
        return Err(SimbaError::new(SimbaErrorTypes::MathError, "`precision` given to `round_precision` should be strictly positive"));
    }
