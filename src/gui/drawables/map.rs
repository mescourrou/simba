use std::{collections::HashMap, path::Path, sync::{Mutex, RwLock}};

use egui::{Color32, Painter, Pos2, Rect, Response, Stroke, Vec2};

use crate::{node_factory::{RobotConfig, RobotRecord}, physics::physic::PhysicConfig, sensors::oriented_landmark_sensor::{OrientedLandmark, OrientedLandmarkSensor}, simulator::SimulatorConfig, utils::time_ordered_data::TimeOrderedData};
...
impl Map {
    pub fn init(path: &String, sim_config: &SimulatorConfig) -> Self {
        ...
        let landmarks = OrientedLandmarkSensor::load_map_from_path(&sim_config.base_path.join(Path::new(path)));
    ...
    pub fn draw(&self, ui: &mut egui::Ui, viewport: &Rect, response: &Response, painter: &Painter, scale: f32) {
        ...
        for landmark in &self.landmarks {
            ...
            painter.rect_filled(Rect::from_center_size(position, Vec2 { x: 0.05*scale, y: 0.05*scale }), 0.01*scale, self.color);
